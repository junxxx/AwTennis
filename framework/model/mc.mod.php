...
function mc_update($uid, $fields){
    ...
    if (empty( $isexists )) {
        ...
        if (is_string($uid_temp)) {
            pdo_update('mc_mapping_fans', array( 'uid' => $insert_id ), array(
                'uniacid' => $_W['uniacid'],
...
function mc_oauth_fans($openid, $acid = 0){
    ...
    $fan = pdo_get('mc_oauth_fans', $condition, array(
        'openid',
        'uid',
    ));
    ...
    if (!empty( $_SESSION['openid'] ) && intval($_W['account']['level']) >= 3) {
        ...
        if (!is_error($userinfo) && !empty( $userinfo ) && is_array($userinfo) && !empty( $userinfo['nickname'] )) {
            ...
            if (!empty( $userinfo['headimgurl'] )) {
                $avator = explode('/', $userinfo['headimgurl']);
                array_pop($avator);
                $userinfo['headimgurl'] = implode('/', $avator) . '/'. 132;
            }
...
function mc_credit_update($uid, $credittype, $creditval = 0, $log = array()){
    ...
    if ($creditval > 0 || ( $value + $creditval >= 0 ) || $credittype == 'credit6') {
        ...
        # 添加微信会员卡的积分/余额同步处理
        $openid = pdo_getcolumn('mc_mapping_fans', array('uid' => $uid , 'uniacid' => $_W['uniacid']), 'openid');

        load()->classs('coupon');
        $cardModel = new MemberCard();
        $wechatCard = $cardModel->holdCard($openid);
        if(!empty($wechatCard)){
            $record = $log[1];
            
            if (empty( $log[1] )) {
                if ($creditval > 0) {
                    $record = $clerk_types[$log[5]] . ': 添加' . $creditval .$credittype_name;
                } else {
                    $record = $clerk_types[$log[5]] . ': 减少' . -$creditval .$credittype_name;
                }
            }

            if($credittype == 'credit1'){
                $cardModel->setRecordBonus(0, $record);
            }
            if($credittype == 'credit2'){
                $cardModel->setRecordBalance(0, $record);
            }
            unset($record);
            $cardModel->updateUser($openid, $wechatCard['card_id'], $wechatCard['code'], true, false);
        }
...
function mc_account_change_operator($clerk_type, $store_id, $clerk_id){
    ...
    if (empty( $stores ) || empty( $clerks )) {
        $clerks = pdo_getall('activity_clerks', array( 'uniacid' => $_W['uniacid'] ), array(
            'id',
            'name',
        ), 'id');
        $stores = pdo_getall('activity_stores', array( 'uniacid' => $_W['uniacid'] ), array(
            'id',
            'business_name',
            'branch_name',
        ), 'id');
...
function mc_fans_groups($force_update = false){
    ...
    $results = pdo_fetchcolumn($sql, array(
        ':uniacid' => $_W['uniacid'],
        ':acid'    => $_W['acid'],
    ));
...
function mc_group_update($uid = 0){
    ...
    if ($uid <= 0) {
        ...
        $user = pdo_fetch('SELECT uid, realname, credit1, credit6, groupid FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND uid = :uid', array(
            ':uniacid' => $_W['uniacid'],
            ':uid'     => $uid,
        ));
        $user['openid'] = pdo_fetchcolumn('SELECT openid FROM ' . tablename('mc_mapping_fans') . ' WHERE acid = :acid AND uid = :uid', array(
            ':acid' => $_W['acid'],
            ':uid'  => $uid,
        ));
    }
    ...
    if ($groupid > 0 && $groupid != $user['groupid']) {
        pdo_update('mc_members', array( 'groupid' => $groupid ), array(
            'uniacid' => $_W['uniacid'],
            'uid'     => $uid,
        ));
...
function mc_init_fans_info($old_fan_info){
    ...
    if (!is_error($fan) && $fan['subscribe'] == 1) {
        ...
        # 增加 unionid 同步
        $record['unionid'] = isset($fan['unionid']) ? $fan['unionid'] : '';
        ...
        if (!empty( $old_fan_info['uid'] )) {
            $user = mc_fetch($old_fan_info['uid'], array(
                'nickname',
                'gender',
                'residecity',
                'resideprovince',
        ...
        ), array( 'fanid' => $old_fan_info['fanid'] ));
    }
...
function mc_card_settings_hide($item = ''){
    ...
    if ($item == 'personal_info') {
        ...
        if (empty( $mcFields['idcard'] ) && empty( $mcFields['height'] ) && empty( $mcFields['weight'] ) && empty( $mcFields['bloodtype'] ) && empty( $mcFields['zodiac'] ) && empty( $mcFields['constellation'] ) && empty( $mcFields['site'] ) && empty( $mcFields['affectivestatus'] ) && empty( $mcFields['lookingfor'] ) && empty( $mcFields['bio'] ) && empty( $mcFields['interest'] ) && empty( $mcFields['telephone'] ) && empty( $mcFields['qq'] ) && empty( $mcFields['msn'] ) && empty( $mcFields['taobao'] ) && empty( $mcFields['alipay'] ) && empty( $mcFields['education'] ) && empty( $mcFields['graduateschool'] ) && empty( $mcFields['studentid'] ) && empty( $mcFields['company'] ) && empty( $mcFields['occupation'] ) && empty( $mcFields['position'] ) && empty( $mcFields['revenue'] ) && empty( $mcFields['avatar'] ) && empty( $mcFields['nickname'] ) && empty( $mcFields['realname'] ) && empty( $mcFields['gender'] ) && empty( $mcFields['birthyear'] ) && empty( $mcFields['resideprovince'] )) {
            return true;
        }
    }
