...
function m($name = ''){
    ...
    $model = AWT_ENROLL_CORE . "model/" . strtolower($name) . '.php';
    if (!is_file($model)) {
        die( ' Model ' . $name . ' Not Found!' );
    }
    require $model;
    $class_name = 'AWT_Tenroll_' . ucfirst($name);
...
function p($name = ''){
    ...
    $model = AWT_ENROLL_PLUGIN . strtolower($name) . '/model.php';
    if (!is_file($model)) {
        return false;
    }
    require $model;
 ...
 * @return bool
 */
function includePluginModel($name){
    $model = AWT_ENROLL_PLUGIN . strtolower($name) . '/model.php';
...
function tpl_form_field_category_3level($name, $parents, $children, $parentid, $childid, $thirdid){
    ...
    if (!empty( $childid ) && !empty( $children[$childid] )) {
        foreach ($children[$childid] as $row) {
            $html .= '<option value="' . $row['id'] . '"' . ( $row['id'] == $thirdid ? 'selected="selected"' : '' ) . '>' . $row['name'] . '</option>';
        }
    }
    $html .= '</select>
		</div>
