...
class AWT_Tenroll_Excel
{
	...
	function export($list, $params = array())
	{
		...
		$excel = new PHPExcel();
		$excel->getProperties()->setCreator("weshine")->setLastModifiedBy("weshine")->setTitle("Office 2007 XLSX Test Document")->setSubject("Office 2007 XLSX Test Document")->setDescription("Test document for Office 2007 XLSX, generated using PHP classes.")->setKeywords("office 2007 openxml php")->setCategory("report file");
		...
		header('Content-Disposition: attachment;filename="' . $params['title'] . '-' . date('Y-m-d H:i', time()) . '.xls"');
	...
	public function import($excefile)
	{
		...
		$path = IA_ROOT . "/addons/awt_enroll/data/virtual/tmp/";
		if (!is_dir($path)) {
			load()->func('file');
			mkdirs($path, '0777');
		}
